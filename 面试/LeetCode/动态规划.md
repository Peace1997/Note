...
1. **最优子结构（Optimal Substructure）**
    - 如果一个问题的最优解可以由其子问题的最优解构造出来，则该问题具有最优子结构。例如，求解斐波那契数列 `F(n) = F(n-1) + F(n-2)` 就是最优子结构的例子。
2. **重叠子问题（Overlapping Subproblems）**
    - 如果一个问题在求解过程中会重复计算相同的子问题，则该问题具有重叠子问题性质。例如，计算 `F(10)` 时需要计算 `F(9)`, `F(8)`, `F(7)` 等，而 `F(9)` 又需要计算 `F(8)`, `F(7)` 等，导致大量重复计算。
3. **状态转移方程（State Transition Equation）**
    - 动态规划的核心是找到问题的 **状态转移方程**，也就是如何从子问题的解推出原问题的解。例如： `F(n) = F(n-1) + F(n-2)`
...
# 状态：当前金额大小需要的硬币数量，希望大小从 amount  -> 0
# 选择：coins中不同金额大小coin
