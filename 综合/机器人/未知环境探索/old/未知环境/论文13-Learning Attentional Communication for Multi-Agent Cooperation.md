...
- **简述**
  ...
  - 注意力单元（**二分类器**）将该特征向量作为输入，输出通信概率。注意力单元是一个**二分类器**，用于**决定智能体是否需要进行通信**，如果需要通信则将其称为 **initiator**（发起者），然后它只会在其观测区域内选择 collaborators，形成 communication group，这里有一个参数 $T$, 表示**接下来的 $T$ 时刻保持该交流组不变**。因为是**所有智能体分享同一个策略网络**，因此相邻的智能体可能具有相似的行为，但是通信可以增加其策略的多样性。
...
![image-20220606223350112](Paper13_2.png)

## 三、 实验验证

在三个协作环境上进行实验，智能体规模都比较大（50、100），验证了ATOC要好于CommNet 、 BiCNet 、 DDPG算法。

![image-20220606223316818](Paper13_3.png)
