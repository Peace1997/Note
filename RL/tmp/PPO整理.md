...
**为什么要采用重要性采样？**
因为是 PPO 算法是 on-policy 的算法，即用于与环境交互的策略与更新的策略是同一策略$\theta$ 。因此用 $\theta$ 采样的数据进行更新，只能更新一次。为了解决利用率较低的问题，可以利用旧策略采样的数据（上一个回合收集的策略）完成新策略的更新。所以 $\theta$ 可以利用 $\theta^k$ 采样的数据完成多次更新。但由于策略已发生变化, 直接使用旧数据估计新策略的优势函数等价值函数时会产生偏差。**重要性采样通过重新加权轨迹数据**, 将其重新适配于新策略的分布, 从而校正这种偏差。
...
> 优势函数为 $A(s,a)$定义为在状态 $s$ 下执行动作 $a$ ，相对于从状态 $s$ 开始的价值函数 $V (s)$ 的回报提升。
>
## rewards to go
...
罗博，针对陈梦婷的简历，我简单整理了几个问题，可以参考一下：
针对第一个项目：

1. 强化学习训练和sim2sim的测试选用什么仿真平台

2. 强化学习的状态、动作空间以及奖励函数是如何设计的

3. 仿真中训练得到的模型可以直接迁移至真机中，其中是如何解决Reality Gap的问题的

4. 强化学习如何与模仿学习进行结合，采用了怎样的模仿学习算法。

针对第二个项目：

1. 强化学习将视觉模块、超声波等多 sensor 信息作为输入，如何进行多模态信息融合的。

2. 四足机器人动态避障、巡航、跟随任务中，分别采用了怎么的算法，不同的算法之间是如何连接的。

综合：

1. 强化学习算法选用哪个repo仓库， 目前常用的强化学习算法是哪个。
