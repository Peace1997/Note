...
PPO (Proximal Policy Optimization) 是一种改进的策略梯度的算法，结合了重要性采样、截断式优化目标（KL 散度）和 GAE（Generalized Advantage Estimator）的思想，对新旧策略的分布进行限制，保证每一次新计算出的策略能够和原策略相差不大，稳步提升算法的表现，降低约束优化问题复杂度。



*PPO解决TRPO存在的问题*
PPO 解决 TRPO **求解约束优化问题复杂度过高**和**重要性采样计算方差很大**的问题。解决方式为：
- **截断式优化目标**
- 悲观的约束：当优势值大于0时，保持足够谨慎的更新；当优势值小于0时，保持足够的惩罚力度。 PPO 1
- 结合其他改进方法（**GAE**）

PPO 算法 1 是解决了 TRPO 算法在使用重要性采样时没有对加权系数进行约束的问题，2 是求解约束优化问题复杂度过高的问题。解决的方法，截断式优化目标和 KL 散度。
...
采用当前策略（更新前的策略，旧策略）去收集数据，用收集到的数据去计算一个更新方向，这个更新方向因为有**重要性采样的校正**，因为**有信赖域（Trust Region）的约束**，因此可以保证策略的提升。
...
（1）新的优化目标
**策略网络**：
基于重要性采样（旧策略）、KL 散度//裁剪（稳步更新；正则化）、优势函数（减少方差）的方法设计了新的优化目标函数，最大化策略梯度来更新策略网络。
**价值网络**：
通过 reward to go 计算每个状态的回报值，通过GAE的方法计算优势值，用于指导价值网络的更新。

（2）重要性采样
PPO 采用了**重要性采样**的方法，通过重新加权旧策略的轨迹数据，纠正新旧策略的偏差，可以利用旧策略采样的数据完成新策略的更新。所以 $\theta$ 可以利用 $\theta^k$ 采样的数据完成多次更新，而用 $\theta$ 采样的数据进行更新，只能更新一次。

在策略优化过程中,我们通常需要根据采样自旧策略的轨迹数据来更新新策略。但由于策略已发生变化,直接使用旧数据估计新策略的优势函数等价值函数时会产生偏差。**重要性采样通过重新加权轨迹数据**,将其重新适配于新策略的分布,从而校正这种偏差。
...
同样是使用重要性采样，PPO 在这里的话是同策略的，这是因为在 PPO 中与环境交互采样数据的策略时${\pi_\theta}_{old}$  , 虽然 PPO 在策略更新时中使用了重要性采样这个技术，通过新旧策略比值产生了加权系数使用旧策略分布去替代新策略分布（新策略分布未知），但是目标策略（待学习的策略）的更新还是使用的${\pi_\theta}_{old}$的轨迹数据，新的策略参数 $\theta$只是优化过程的中间产物，并没有用于采样新的轨迹，因此采样策略更目标策略是同一策略。

在 PPO 中,我们计算策略梯度来更新策略网络的参数。这个策略梯度是基于当前策略π与环境交互采样得到的轨迹数据计算得到的。也就是说,梯度更新依赖于采样自当前策略的行为数据。这符合了 on-policy 算法的定义。虽然PPO也使用了重要性采样的技术,但这种重要性采样主要是为了**降低策略梯度估计的方差**,而不是改变原始数据分布。换句话说,即使使用了重要性采样,PPO的梯度估计仍然是在当前策略π的数据分布上进行的,而不是任意其他策略的分布。

PPO需要在每次策略更新后重新与环境交互以采集新的数据,因为之前采样得到的数据来自旧策略,无法直接用于新策略的评估。这种基于每次策略改变需要重新采样的特点,也反映了PPO的on-policy属性。

[[_强化学习/常问问题#行为策略 目标策略]]
## 2. 目标函数约束
...
\hat{A}_t^{(2)}:=\delta_t^V+\gamma \delta_{t+1}^V & =-V\left(s_t\right)+r_t+\gamma r_{t+1}+\gamma^2 V\left(s_{t+2}\right)=  -V\left(s_t\right)+r_t+ \gamma V(s_{t+1}) + \gamma r_{t+1}+\gamma^2 V\left(s_{t+2}\right) -\gamma V(s_{t+1})  \ \\
...
\end{aligned}
$$
然后运用 GAE 公式进行**优势函数**的估计：（**指数加权平均法**）$\sigma$
#指数加权平均
$$
\begin{aligned}
