...
1. 分离策略和价值函数：利用这两个模块的独立性提高算法的效率，提高算法的扩展性
2. 新的优化目标：
	- 对于策略网络：基于重要性采样、优势函数、KL 散度//裁剪（防止不同分布差距过大；正则化）的方法设计了新的优化目标函数，最大化策略梯度来更新策略网络。
	- 对于价值网络：通过 reward to go 计算每个状态的回报值，用于指导价值网络的更新


PPO采用了**重要性采样**的方法， 可以利用旧策略采样的数据完成新策略的更新。所以 $\theta$ 可以利用 $\theta^k$ 采样的数据完成多次更新。
为了避免不同分布之间差异过大，可以通过**KL 散度**、**裁剪**的方法来对分布差异进行纠正。

> 通过引入新的正则化（KL 散度、裁剪）的技巧，可以很好提高算法的稳健性。
> 重要性采样：使用另外一种分布，来逼近所求分布的一种方法
> KL 散度//裁剪：避免新旧策略
...
PPO 的核心理念就是避免 TRPO 算法中计算 KL 散度导致的复杂性。而TRPO算法就是为了解决参数迭代过程中步长过小和过大造成的问题，从参数空间的迭代直接进入到策略空间的迭代，并保证小步单调上升的迭代。
