...
【神经网络】：使用了神经网络来进行值函数的逼近 
【目标Q网络】：使用了**target Q network**来更新 target ：Target Q 使用了单独的网络来单独处理TD算法的TD偏差
【经验回放池】： 使用**经验回放池**（Experience Replay）：agent在与环境交互的时候 将交互数据存放在一个库里面，然后训练的时候 从中随机采样数据进行训练；
