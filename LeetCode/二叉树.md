...
![image-20220717154859553](img/OrderTraverse)
...
def quicksort(nums,left,right):
    ...
    p = partation(nums,left,right)    # 找分割点
    # 	p 就是其中的中间分割点
...
def partation(nums,left,right):
    i,j = left,right
    while i<j:
        # 因为是最左边元素为基准，所以要从右边开始搜索。如果先从左边开始搜索，则右边搜索时可能无法找到比基准小的元素，而从右边开始搜索时，左边搜索时即使找不到比基准大的也没有关系，即确定这些元素都比基准元素小，可以直接确定分割点。			https://segmentfault.com/a/1190000015396497
