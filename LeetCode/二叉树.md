...
二叉树的所有问题，就是让你在**前中后序位置**注入巧妙的代码逻辑，去达到自己的目的，你只需要单独思考**每一个节点**应该做什么，其他的不用你管，抛给二叉树遍历框架，**递归会在所有节点上做相同的操作**。
...
# 	- 在前序位置做时，你在进入该节点时，左右子树还没开始交换，就将左右子树进行交换（从上往下交换）
# 	- 在后序位置做时，左右子树都交换了，你在离开节点时，再将左右子树进行交换（从下往上交换）
...
# 利用队列先进先出的特性，模拟二叉树的层次遍历
# 整体思路，从队列输出一个元素就把该元素对应的左右子节点加到队列末尾
