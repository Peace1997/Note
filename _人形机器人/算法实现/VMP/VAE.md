...
影响 VAE 网络训练的主要核心因素：
- Motion Embedding 长度，即 Encoder 的输出维度
- Window length ，即窗口长度
- 损失的权重，不同损失项所占的权重不同，对最后的结果会有不同的结果
- 参考数据的质量
...
当采用 64 dim 的 motion embedding 时，重构后的机身方向一直抖动，其他高度和关节位置跟踪是比较正常的，产生这样的原因可能有四个：
-  **motion embedding 的维度过少**：不足以完整表达运动特征
- **参考数据窗口过小**：当前参考数据为 120 hz，训练时 30 帧的窗口只能表达 0.25 秒的运动特征，当同一轨迹的机身如果经常会改变，可能无法很好捕捉这一特征
- **重构损失中机身 Orientation 项的权重较少：**，从而忽略了机身 Orientation 的表达。（归一化后可能并没有）
- **参考数据没有经过筛选：**，参考数据未经筛选，会导致部分

|       | Base h_err      | **Base** **ori_err**    | Joint pos_err   |
| ----- | --------------- | ----------------------- | --------------- |
| Squat | 0.0008 ± 0.0019 | **0.3048 rad ± 0.2526** | 0.0033 ± 0.0023 |
| Jump  | 0.0035 ± 0.0066 | **0.1127 rad ± 0.0538** | 0.0036 ± 0.0045 |
