...
## Multi-Gait：单一策略实现多种风格运动

`Gait-Conditioned Reinforcement Learning with Multi-Phase Curriculum for Humanoid Locomotion` - 2025.5

本文提出一个基于步态条件变化奖励路由和分阶段课程学习的强化学习框架，无需运动捕捉参考数据，通过单一策略网络实现人形机器人机器人自然的多步态（站立、行走、跑步）控制及平滑过渡。

***Contribution***：
- **动态奖励路由（Reward Routing）**：以步态 ID 作为条件信号（one-hot），在不同的步态仅激活相应的奖励函数子项。如果混合所有奖励，策略容易陷入局部最优。
- **仿生奖励**：无运动参考数据集，直接通过生物力学（直膝支撑、臂腿协调摆动）生成自然运动。
- **多阶段课程学习**：分阶段引入步态复杂性和指令空间，
	- 阶段 1：先进行低速行走训练，专注于节奏稳定性、足部离地高度、直膝支撑。
	- 阶段 2：实现静态站立与动态行走的平滑控制，
	- 阶段 3：重点学习高动态步伐的爆发力和腾空控制。
- **单一策略实现多步态切换**：使用 LSTM 实现多步态切换过渡，简化架构降低延迟。
