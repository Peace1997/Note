下面是一套基于 MIT 常用的 MPC（Model Predictive Control）框架，用于**六自由度人形机器人**末端足底接触力规划，以实现机器人对设定速度的跟踪。该方案包含从运动学到动力学建模、再到 MPC 求解和实时执行的完整实现思路。为方便说明，以下步骤基于世界坐标系进行解释，但在实际实现时可根据需要在局部坐标系与世界坐标系之间转换。
...
1. **机器人本体：** 具有六个可动关节（或简化为六个关键自由度）的双足人形机器人。
2. **控制目标：**
...
1. **世界坐标系（WCS）：** 记为 $\{W\}$，固定在地面上，用于描述机器人的绝对位置和姿态。
2. **机体坐标系（BCS）：** 记为 $\{B\}$，通常放置在机器人躯干质心或基座。
3. **足底坐标系：** 记为 $\{F_i\}$，与每只脚的触地点相关。
...
其中：
1. $M (q)$ 为惯性矩阵，随关节角度 $q$ 变化。
2. $C (q, \dot{q})$ 表示 Coriolis 和离心力。
3. $g (q)$ 表示重力项。
4. $\tau$ 为关节力矩向量（控制输入）。
5. $F_{c_i}$ 表示某只脚在接触地面时的足底接触力（包含三维力和相应的力矩，也可拆分成力与力矩两部分）。
6. $J_i (q)$ 为足底接触点对应的雅可比矩阵，用于将足底坐标系中的力转换到关节空间。

### 3.2 足底接触模型

1. **接触力表达：** 对于每只脚，若其与地面接触，则可在世界坐标系下表示为
   $$
   F_{c_i} = 
   \begin{bmatrix}
   F_{x_i} \\
...
1. $x_{ref, k}$ 为期望状态（可包含期望步态，或者在本例中可以是期望质心速度等）。
2. $u_{ref, k}$ 为参考控制输入（若有）。
3. $Q, R, P$ 分别为状态偏差、控制偏差与终端偏差的权重矩阵。在行走速度跟踪场景下，可在 $Q$ 中加大“线速度”相关项的权重。
...
1. **实时更新状态：** 通过传感器（关节编码器、IMU、力传感器等），获取当前实际状态 $x_k$。
2. **线性化：** 在 $(x_k, u_{k-1})$ 附近做一阶近似，生成线性化后的系统矩阵 $A_k, B_k$。
3. **预测与求解：** 在时域 $k, k+1, \dots, k+N-1$ 内最小化目标函数，求得控制输入序列 $\{u_k^*, \dots, u_{k+N-1}^*\}$。
4. **应用第一个控制量：** 在实际系统上应用 $u_k^*$，使机器人执行对应的力矩和足底力分配。
5. **滚动优化**：将时刻 $k+1$ 的状态更新为新的初始条件，重复上述步骤。

---

## 7. 框架整合与实施要点

1. **初始步态/状态生成**：提前给定初始支撑腿状态和足底力，以便第一步优化有合理初始值。
2. **状态/观测融合**：IMU、关节角、力传感器等信息需要经过滤波（如 EKF）融合，以得到平滑且精确的机器人质心、速度等估计。
3. **脚踝与足底传感**：在单足支撑与双足支撑的切换过程中，要及时更新接触力变量和相关约束。
4. **并行运算**：若使用高阶或长时域的 MPC，需充分利用多核 CPU 或 GPU 进行并行计算，以满足实时性。
5. **故障安全**：在实际的机器人系统中，若优化器出现求解不成功或传感器失效，应有安全策略，如退回既定安全姿态或切换到 PID 模式等。
