...
沿着时间轴滑动，只对序列的**时间维度**进行卷积。处理的数据通常是时序数据，例如 10 帧的机器人关节数据，这时卷积核

| 项目                     | 含义                           |
| ---------------------- | ---------------------------- |
| 输入通道数 (`in_channels`)  | 对应每个时间帧的特征维度，                |
| 卷积核大小 (`kernel_size`)  | 表示沿时间维度一次聚合多少帧的信息（即时间窗口大小）   |
| 输出通道数 (`out_channels`) | 表示提取的特征个数，模型可自由设定，例如 32、64 等 |
