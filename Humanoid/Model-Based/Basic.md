...
**1. 距离的平方效应**
- $J = m \cdot r^2$ ；在质量分布到旋转轴的距离$r$，所以距离的影响远大于重量。
- 人形机器人的“大腿”总是比“小腿”粗壮，就是为了把沉重的电机尽量靠近躯干（减小 $r$），从而降低腿部摆动时的惯量，让走路更省力、频率更高。所以末端的负载对
...
**3. 耦合性**
- 关节之间存在惯量耦合，一个关节的运动会产生惯性力作用在另一个关节上。
- 例如：快速挥动大臂，小臂即使不发力也会因为惯性甩出去。在机器人控制中，这叫“动力学解耦”，需要算法去抵消这种干扰。

在人形机器人中，几乎所有的动作（挥臂、踢腿、转头）都是绕着关节轴做**旋转运动**。因此，通常口中的“惯量”，百分之九十指的都是“转动惯量”
...
惯量矩阵 $M(q)$ 是关节（角度向量 $q$）的函数。随着机器人运动，其各个部位的几何位置不断变化，导致系统的质量分布相对旋转轴发生改变，因此**惯量在时间轴上是不断波动的**。
...
减速比（通常用 $i$ 表示）是减速机构的输入角速度（电机转速）与输出角速度（关节转速）的比值。

$$i = \frac{\omega_{in}}{\omega_{out}} = \frac{n_{motor}}{n_{joint}}$$
如果不考虑摩擦损耗，根据功率守恒定律（$P = T \cdot \omega$），速度降低多少倍，扭矩就提升多少倍。
- **速度降低：** 关节转得比电机慢。
- **扭矩放大：** 关节的力量比电机输出的力量大。
...
***1. 扭矩放大器***
人形机器人的电机通常只有几牛米（N·m）的力气，这连提一桶水都费劲。通过减速比： 如果减速比是 100:1，原本只能提起 1kg 的电机，现在理论上可以提起 100kg。这让机器人能够支撑体重、负重行走。

***2. 惯量匹配***
减速比越大，电机控制负载就越容易，系统越稳定，不会因为负载太重而产生震荡。
- **折算效应：** 负载（手臂）的惯量在电机看来，会随着减速比的平方倍缩小。
- **等效惯量：** $J_{equivalent} = \frac{J_{load}}{i^2}$。

***3. 提高分辨率***

由于减速比的存在，关节移动 1 度，电机可能已经转了 100 度。这意味着电机端的光学编码器（传感器）可以实现极高精度的位置反馈，让机器人的动作更细腻。

***4. 副作用***
- **牺牲速度：** 减速比越高，机器人的极限动作速度就越慢（比如无法完成高速奔跑）。
- **增大背隙（Backlash）：** 齿轮之间有缝隙。减速比越高，级数越多，累积的缝隙可能导致末端抖动。
- **降低“透明度”（Backdriveability）：** 高减速比（>50:1）：人推不动机器人的手，碰撞时容易由于惯性放大而伤人；低减速比（<10:1）： “准直驱”方案。人可以轻巧地拨动机器人的肢体，安全性极高。
